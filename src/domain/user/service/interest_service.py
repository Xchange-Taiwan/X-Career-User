...
class InterestService:
    def __init__(self, interest_repository: InterestRepository, cache: LocalCache):
        self.__interest_repository: InterestRepository = interest_repository
        # 常數資料可用 local cache
        self.cache = cache

    async def get_all_interest(
        self, db: AsyncSession, interest: InterestCategory, language: Language
    ) -> InterestListVO:
        try:
            ...
            query: List[Type[Interest]] = (
                await self.__interest_repository.get_all_interest(
                    db, interest, language.value
                )
            )

            interests: List[InterestVO] = [
                self.convert_to_interest_vo(interest) for interest in query
            ]
    ...
    async def get_by_interest_category(
        self, db: AsyncSession, interest: InterestCategory
    ) -> InterestVO:
        try:
            return self.convert_to_interest_vo(
                await self.__interest_repository.get_by_interest(db, interest)
            )
        except Exception as e:
            log.error("get_by_interest_category error: %s", str(e))
            raise_http_exception(e, msg="Internal Server Error")

    async def get_interest_by_ids(
        self, db: AsyncSession, ids: List[int]
    ) -> InterestListVO:
        try:
            query: List[Type[Interest]] = (
                await self.__interest_repository.get_interest_by_ids(db, ids)
            )
    ...
    async def get_interest_by_id(
        self, db: AsyncSession, interest_id: int
    ) -> InterestVO:
        try:
            query: Type[Interest] = await self.__interest_repository.get_interest_by_id(
                db, interest_id
            )
    ...
    async def get_by_subject_group_and_language(
        self,
        db: AsyncSession,
        subject_groups: List[str],
        language: str = DEFAULT_LANGUAGE,
    ) -> Optional[InterestListVO]:
        try:
            ...
            all_list_vo: InterestListVO = self.convert_to_interest_list_vo(
                interest_list
            )
            # set local cache
            await self.cache.set(cache_key, all_list_vo, CACHE_TTL)
    ...
    def filter_by_subject_group(
        self, list_vo: InterestListVO, subject_groups: List[str]
    ) -> InterestListVO:
        sub_list_vo = InterestListVO(
            interests=[],
            language=list_vo.language,
        )
        sub_list_vo.interests = [
            i for i in list_vo.interests if i.subject_group in subject_groups
        ]
        return sub_list_vo
